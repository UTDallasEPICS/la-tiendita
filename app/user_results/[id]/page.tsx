  ...
  async function getSurveysAndResults() {
    try {
      ...
      const responses: Response[] = await Promise.all([
        fetch(`${BASE_URL}/surveys`), fetch(`${BASE_URL}/survey_results`)
      ])
      // Set the data 
  ...
  return (
    <div className="mb-10 text-text">
      ...
      <div className="flex flex-col gap-6 mt-2">
        {surveys.map(survey =>
          <FormResult key={survey.id} survey={survey} result={getQueriedResult(survey)} />
        )}
      </div>
    </div>
  )
