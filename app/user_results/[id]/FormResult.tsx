...
function AnswerPortion({
  ...
  return (
    <div
      ...
      style={questionType !== "FREE_RESPONSE" ? { alignItems: "center" } : { alignItems: "baseline" }}
    >
      ...
      <div className="px-4">
        {detailPresent && (
          <div className="px-2 pb-4">
            ...
            {(result.answersData as Record<string, any>[]).map((data, index) =>
              <div key={index}>
                <QuestionPortion index={index + 1} question={data.question.questionString} />
                <AnswerPortion
                  questionType={data.question.type}
                  maxScale={data.question.maxValue} optionsMap={data.question.optionsMap}
                  answer={data.answer}
                />
              </div>
            )}
