...
function QuestionWithChoice({
  index,
  question,
  questionChartData,
}: QuestionWithChoiceProps) {
  ...
  function renderChoice() {
    if (question.type === "MULTIPLE_CHOICE") {
      const optionsMap = question.optionsMap as Record<
        string,
      ...
      return (
        <div>
          {Object.keys(optionsMap).map((option) => (
            ...
            >
              <div className="px-2">
                <div className="text-lg text-white w-7 h-7 flex justify-center rounded-full bg-accent shadow-md">
                  {option}
                </div>
              </div>
                ...
                <div className="grid grid-cols-2  w-full">
                  <div>
                    ...
                    <span className="font-semibold">
                      {optionsMap[option].category}
                    </span>
                  </div>
                  <div>
                    <span className="text-sm">Weight:</span>{" "}
                    <span className="font-semibold">
                      {optionsMap[option].weight}
                    </span>
                  </div>
                </div>
              </div>
      ...
      return (
        <div>
          {range.map((scaleValue) => (
            <div
              key={scaleValue}
              className="mt-4 flex items-center gap-4 rounded-md text-lg"
            >
              <div className="w-7 h-7 flex justify-center rounded-full bg-accent shadow-md text-white">
                {scaleValue}
              </div>
      ...
      return (
        <div className="text-lg">
          Answer will be analyzed with the help of LLM.
        </div>
      );
    }
  }
...
function QuestionListPortion({
  surveyID,
  totalChartData,
}: QuestionListPortionProps) {
  ...
  return (
    <>
      <tr className="bg-white border-t border-primary text-text text-center">
        ...
        <td style={{ width: "20%" }}>
          {survey.createdAt.toString().slice(0, 10)}
        </td>
        <td style={{ width: "20%" }}>
          {survey.updatedAt.toString().slice(0, 10)}
        </td>
        <td style={{ width: "10%" }}>
          <button
    ...
    <div className="w-[80%] mt-10 mx-auto">
      <div>
        <h1 className="text-2xl font-bold text-text">
          List of surveys ({surveyList.length}):
        </h1>
      </div>
        ...
        <thead>
          <tr className="bg-primary text-white text-center text-gray-700">
            {[
              ...
              <th key={field} className="py-2 text-lg">
                {field}
              </th>
            ))}
          </tr>
